<template>
    <div class="container">
     <div v-if="profileAdmin" class="toggle-edit">
      ...
      <h1>THE UPCOMMING SEASON</h1>
    <!-- <CarouselView /> -->
     <section id ="product1" class="section-p1">
      <div v-if="upcommingSeason.length">
        <div class="pro-container" style="display:flex">
            <ProductListItem
            v-for="(productItem, index) in upcommingSeason"
            :key="index"
      ...
      <div v-else>
        <h3>This Offer Is Currently Not Available</h3>
      </div>
    </section>
    </div>
</template>
<script>
    ...
    export default {
      ...
      computed: {
        upcommingSeason() {
           return this.$store.state.upcommingSeason;
        },
        editProducts: {
         get() {
      ...
      created() {
        this.$store.dispatch('getUpcommingSeason');
      }
    };
</script>
<style scoped>
