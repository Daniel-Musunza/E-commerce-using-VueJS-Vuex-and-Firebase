<template>
    ...
    <!-- <CarouselView /> -->
     <section id ="product1" class="section-p1">
      <div v-if="repair.length">
        <div class="pro-container" style="display:flex">
            <ProductListItem
            v-for="(productItem, index) in repair"
            :key="index"
      ...
      <div v-else>
        <h3>Repair services are Currently not Availlable</h3>
      </div>
    </section>
    </div>
</template>
<script>
    ...
    export default {
      ...
      computed: {
        repair() {
           return this.$store.state.repair;
        },
        editProducts: {
         get() {
